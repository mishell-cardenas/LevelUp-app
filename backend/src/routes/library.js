...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Game added: ${gameName} for user ${req.session.user.username}`,
    );
...
router.patch("/:id", async (req, res) => {
  ...
  try {
    ...
    if (priority !== undefined)
      updateFields.priority = priority ? parseInt(priority) : null;
    if (progressPercent !== undefined)
      updateFields.progressPercent = parseInt(progressPercent) || 0;
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Game deleted: ${game.gameName} (${journalResult.deletedCount} journal entries removed)`,
    );
    res.json({
      success: true,
