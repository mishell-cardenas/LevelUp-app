...
async function loadJournal() {
  try {
    ...
    if (data.game) {
      document.getElementById('journalGameName').textContent = `${data.game.gameName} - Journal`;
    }
...
function renderEntries() {
  ...
  if (journalEntries.length === 0) {
    list.innerHTML = '<p style="color: rgba(255,255,255,0.5); text-align:center;">No entries yet. Add your first session!</p>';
    return;
  }
...
function startEdit(entryId) {
  ...
  document.getElementById('currentObjectives').value = entry.currentObjectives || '';
  document.getElementById('importantDetails').value = entry.importantDetails || '';
