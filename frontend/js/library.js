...
function renderLibrary() {
  ...
  if (libraryGames.length === 0) {
    grid.innerHTML = '<p style="color: rgba(255,255,255,0.5); text-align:center; grid-column:1/-1;">Your library is empty. Click + to add a game!</p>';
    return;
  }

  grid.innerHTML = libraryGames.map(game => `
...
function openGameModal(cardElement) {
  ...
  document.getElementById('modalStatus').value = currentGame.status || 'Wishlist';
  document.getElementById('modalCompletion').value = currentGame.progressPercent || 0;
  document.getElementById('modalPriority').value = currentGame.priority || '';
  document.getElementById('modalPlatform').value = currentGame.platform || 'Steam';
...
async function deleteGame() {
  ...
  try {
    const res = await fetch(`/api/library/${currentGame._id}`, { method: 'DELETE' });
    const data = await res.json();
  ...
  input.addEventListener('input', () => {
    ...
    searchTimeout = setTimeout(async () => {
      try {
        ...
        if (games.length === 0) {
          resultsDiv.innerHTML = '<div style="padding:8px 12px; color:rgba(255,255,255,0.5);">No games found</div>';
        ...
        resultsDiv.querySelectorAll('.search-result-item').forEach(item => {
          item.addEventListener('click', () => {
            ...
            document.getElementById('gameHeaderImage').value = item.dataset.image;
            resultsDiv.style.display = 'none';
          });
        });
...
async function handleAddGame(e) {
  ...
  try {
    const res = await fetch('/api/library', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ steamId, gameName, headerImage, status, platform, priority, progressPercent })
    });
...
function setupEventListeners() {
  // Filters
  document.getElementById('statusFilter').addEventListener('change', loadLibrary);
